...
namespace GameLauncher.Views
{
    public partial class PosterView : UserControl
    {
        ...
        {
            CollectionViewSource GameListCV = (CollectionViewSource)FindResource("GameListCVS");
            if (FilterGenreName != null) { GameListCV.Filter += new FilterEventHandler(GenreFilter); }
            if (GameSearchBar.Text != "") { GameListCV.Filter += new FilterEventHandler(GameSearch); }
            if (GameListCV.View != null) //This is getting a null "GameListCVS.View" on genre only, works if searchbar updated
